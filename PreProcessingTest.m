...
perforation = zeros(length(imgStats));
...
%         plot(xq(~in),yq(~in),'bo') % points outside
    ...
    if ((numel(xq(in)) == numel(xq(on))) && numel(xq(~in)) == 0)
       segment(i) = i;   
       perforation(i) = i;
%        plot(xq(in&~on),yq(in&~on),'r+') % points strictly inside        
    
    elseif (numel(xq(~in)) > 10 || numel(xq(on))> 75)
        segment(i) = i;     
%         plot(xq(on),yq(on),'g*') % points on edge
%         plot(xq(~in),yq(~in),'bo') % points outside
    end
...
perforation = nonzeros(perforation);
disp("PERFORATION:");
disp(perforation);
disp("Segment:");
disp(segment);
...
% bwl2 = sum(reshape(cell2mat(arrayfun(@(y)y==imgStats,perforation,'un',0)),size(x,1),size(x,2),[]),3);
% for i = 1:length(perforation)
%     zImg = imgStats(perforation(i)).PixelList;
%     h=drawpoint("Position",PixelList);
%     zMask = createMask(h);
%     bwl2 = bwmorph(zMask,'dilate',1);
%     z = x | bwl2;
% end
% 
...
% 
% zCon = bwconncomp(z);
% zNum = zCon.NumObjects;
% zStats = regionprops(zCon, 'all');
% zBox = [zStats.BoundingBox];
% zBox = reshape(zBox, [4 zNum]);
% % figure(10); imshow(~xcl);
% % Display connected components
% 
% perforation = zeros(zNum);
% 
% figure(4);
% imshow(bw);
% hold on;
% for i= 1:zNum    
%     imgPxList = zStats(i).PixelList;
%     xq = imgPxList(:,1);
%     yq = imgPxList(:,2);
% 
%     % Determine if CC is within the Mask
...
%     if ((numel(xq(in)) == numel(xq(on))) && numel(xq(~in)) ==0)
%         perforation(i) = i;
%         rectangle('position',zBox(:,i), EdgeColor='r');        
%     end
% end
% 
% perforation = nonzeros(perforation);
...
% disp("BREAK");
% 
% if (~isempty(perforation))
%     for i = 1:length(perforation)
%         perImg = zStats(perforation(i)).Image;
%         for j = 1:length(segment)
%            segImg = imgStats(segment(j)).Image;
%            if (ismember(segImg, perImg))
%                disp("N:"+j);
%                segment(j) = 0;
%            end
%         end
...
% for i = 1:length(perforation)
%     bounds = zStats(perforation(i)).BoundingBox;
%     rectangle('position', bounds, EdgeColor='r');
%     text(bounds(1),bounds(2),"Perforation", 'Color', 'g');
% end
% hold off;